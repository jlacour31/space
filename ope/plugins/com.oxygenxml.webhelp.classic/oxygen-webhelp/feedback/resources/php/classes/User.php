...
class User
{
    ...
    function updateProfile($profileInfo)
    {
        ...
        if (!$profileInfo['editByAdmin']) {
            ...
            if (strpos($profileInfo['userId'], 'ldap_') === 0) {
                ...
                if ($toReturn) {
                    $query = "SELECT userId FROM users WHERE userName='" . $db->sanitize($userName) . "'";
                    $db->Open($query);
                    $db->MoveNext();
     ...
     *
     * @param string ids to be deleted (1,2,3...)
     */
    function delete($ids)
    {
        $idsArray = preg_split('/,/', $ids);
     ...
     *
     * @return multitype:String productId=>Name
     */
