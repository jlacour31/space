...
class Comment
{
    ...
    function moderate($id, $state)
    {
        ...
        // Check that $state is one of permitted values
        if ( $state != 'deleted' && $state != 'new' && $state != 'approved' && $state != 'rejected' )
            return $toReturn;
...
// 				$db->Run($query0);
    ...
    function update($info)
    {
        $toReturn = array();
        $db = new RecordSet($this->dbConnectionInfo);
        $commentId = (isset($info['editedId']) && (strlen(trim($info['editedId'])) > 0 && is_numeric($info['editedId'])) ? $db->sanitize(trim($info['editedId'])) : -1);
    ...
    function listForPage($info, $refId = -1, $isForAdmin = false)
    {
        ...
        $product = $db->sanitize($info['product']);
        $version = $db->sanitize($info['version']);
        $idUser = $db->sanitize($info['userId']);
    ...
    function setMinimalVisibleVersion($product, $minVersion, $version)
    {
        ...
        $product = $db->sanitize($product);
        $minVersion = $db->sanitize($minVersion);
        $version = $db->sanitize($version);

        $toReturn = true;
        if ($this->isLoggedModerator()) {
            $selectQ = "SELECT value FROM webhelp WHERE parameter='minVisibleVersion' AND product='" . $product. "' AND version='" . $version . "';";
            if ($db->Run($selectQ) > 0) {
                $query = "UPDATE webhelp SET value = '" . $minVersion . "' WHERE parameter='minVisibleVersion' AND product='" . $product . "' AND version='" . $version . "';";
                $db->Run($query);
            } else {
                $query = "INSERT INTO webhelp (parameter, value, product, version) VALUES ('minVisibleVersion', '" . $minVersion . "', '" . $product . "', '" . $version . "')";
                $db->Run($query);
            }
        } else {
            $toReturn = false;
