...
A mathphrase is a simple, inline equation, one that can be represented using ordinary text, symbols, subscripts, and superscripts.
Category: Mathematics-->
...
<![%termdef.element;[
<!--doc:A termdef is an inline term definition. Some styles of documentation collect all terms together in a Glossary of some sort, but another style is to place the definitions inline.-->
